...
export default function SwipeScreen() {
  ...
  // Fetch recipe stack
  const { data: recipes, isLoading, refetch } = trpc.recipe.getRecipeStack.useQuery(
    { limit: 10 },
    {
  ...
  const likeMutation = trpc.recipe.likeRecipe.useMutation({
    onSuccess: (data) => {
      if (data.matched && data.recipe) {
        Alert.alert(
          '🎉 It\'s a Match!',
          `You and your partner both like "${data.recipe.title}"!`,
  ...
  const handleSwipe = useCallback((isLike: boolean) => {
  ...
  }, [currentRecipe, currentIndex, recipes, likeMutation, refetch, translateX, translateY]);
  ...
  if (!recipes || recipes.length === 0) {
    return (
      <View className="flex-1 items-center justify-center bg-gray-50 px-8">
        ...
        <Text className="text-xl font-bold text-gray-900 mt-4">No Recipes Available</Text>
  ...
  if (currentIndex >= recipes.length) {
    return (
      <View className="flex-1 items-center justify-center bg-gray-50 px-8">
        ...
        <Text className="text-xl font-bold text-gray-900 mt-4">All Caught Up!</Text>
        <Text className="text-gray-600 text-center mt-2">
          You've seen all available recipes. Check your matches or come back later!
        </Text>
        <Pressable
          onPress={() => {
            setCurrentIndex(0);
    ...
    <SafeAreaView className="flex-1 bg-gray-50">
      <View className="flex-1">
        ...
        <View className="px-6 py-4">
          ...
          <Text className="text-gray-600 mt-1">Swipe to find recipes you both love</Text>
        </View>
