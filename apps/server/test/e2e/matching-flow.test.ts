...
describe("E2E: Recipe Matching Flow", () => {
	...
	async function setupTestData() {
		...
		recipe2 = await Recipe.create({
			...
			steps: ["Cook beef", "Warm taco shells", "Assemble tacos"],
			tags: ["mexican", "quick"],
			nutritionInfo: {
				calories: 380,
				protein: 28,
	...
	describe("Complete Matching Flow", () => {
		it("should complete full matching flow from partner connection to match creation", async () => {
			...
			expect(matchesA.matches[0]._id.toString()).toBe(
				swipeB2.matchId.toString(),
			);
			expect(matchesB.matches[0]._id.toString()).toBe(
				swipeB2.matchId.toString(),
			);
		...
		it("should handle match status updates through cooking lifecycle", async () => {
			...
			expect(cookedHistory.matches[0]._id.toString()).toBe(
				match._id.toString(),
			);
		});
