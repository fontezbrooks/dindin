...
const MONGODB_TEST_URI =
	...
	"mongodb://localhost:27017/dindin_favorites_test";

beforeAll(async () => {
	// Connect to test database
	try {
		await mongoose.connect(MONGODB_TEST_URI);
		logger.log("Connected to test database for favorites tests");

		// Ensure clean state
		await mongoose.connection.db.dropDatabase();
		logger.log("Test database cleaned");
	} catch (error) {
...
afterAll(async () => {
	try {
		...
		await mongoose.connection.db.dropDatabase();
		logger.log("Test database cleaned up");

		// Close connection
		await mongoose.connection.close();
		logger.log("Disconnected from test database");
	} catch (error) {
