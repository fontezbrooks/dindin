...
import { FavoritesOperationContext } from '../../src/types/favorites.types';
...
describe('FavoritesService', () => {
  ...
  beforeAll(async () => {
    ...
    const MONGODB_URI = process.env.MONGODB_TEST_URI || 'mongodb://localhost:27017/dindin_test';
  ...
  describe('addToFavorites', () => {
    it('should successfully add recipe to favorites', async () => {
      const result = await favoritesService.addToFavorites(context, testRecipe._id.toString());
      ...
      expect(updatedUser?.likedRecipes[0].toString()).toBe(testRecipe._id.toString());
    ...
    it('should throw error when recipe already in favorites', async () => {
      ...
      try {
        await favoritesService.addToFavorites(context, testRecipe._id.toString());
      } catch (error) {
        expect(error).toBeInstanceOf(TRPCError);
        expect((error as TRPCError).code).toBe('BAD_REQUEST');
        expect((error as TRPCError).message).toBe('Recipe already in favorites');
      }
    });
    ...
    it('should throw error when user does not exist', async () => {
      ...
      await expect(
        favoritesService.addToFavorites(invalidContext, testRecipe._id.toString())
      ).rejects.toThrow(TRPCError);

      try {
        await favoritesService.addToFavorites(invalidContext, testRecipe._id.toString());
      } catch (error) {
        expect(error).toBeInstanceOf(TRPCError);
        expect((error as TRPCError).code).toBe('BAD_REQUEST');
  ...
  describe('removeFromFavorites', () => {
    ...
    it('should successfully remove recipe from favorites', async () => {
      const result = await favoritesService.removeFromFavorites(context, testRecipe._id.toString());
    ...
    it('should throw error when recipe not in favorites', async () => {
      ...
      await expect(
        favoritesService.removeFromFavorites(context, testRecipe._id.toString())
      ).rejects.toThrow(TRPCError);

      try {
        await favoritesService.removeFromFavorites(context, testRecipe._id.toString());
      } catch (error) {
        expect(error).toBeInstanceOf(TRPCError);
        expect((error as TRPCError).code).toBe('NOT_FOUND');
        expect((error as TRPCError).message).toBe('Recipe not found in favorites');
      }
    });
    ...
    it('should handle multiple recipes in favorites correctly', async () => {
      ...
      const secondRecipe = await Recipe.create({
        ...
        cuisine: ['Mexican'],
        ingredients: [
          { name: 'Test Ingredient 2', amount: '2', unit: 'cups' }
        ],
        instructions: [
          { step: 1, description: 'Second test instruction' }
        ],
        tags: ['test2'],
        servings: 4,
        likes: 0,
      ...
      const updatedUser = await DindinUser.findById(testUser._id);
      expect(updatedUser?.likedRecipes).toHaveLength(1);
      expect(updatedUser?.likedRecipes[0].toString()).toBe(secondRecipe._id.toString());
    });
  });
  ...
  describe('isFavorite', () => {
    it('should return true when recipe is in favorites', async () => {
      ...
      const result = await favoritesService.isFavorite(context, testRecipe._id.toString());
  ...
  describe('getFavoritesCount', () => {
    ...
    it('should return 0 for non-existent user', async () => {
      const count = await favoritesService.getFavoritesCount('non-existent-user');
      expect(count).toBe(0);
    });
  });
  ...
  describe('areFavorites', () => {
    ...
    it('should correctly check multiple recipes', async () => {
      ...
      const result = await favoritesService.areFavorites(
        testUser.authUserId,
        [testRecipe._id.toString(), anotherRecipe._id.toString()]
      );
    ...
    it('should handle invalid recipe IDs gracefully', async () => {
      const result = await favoritesService.areFavorites(
        testUser.authUserId,
        ['invalid-id', testRecipe._id.toString()]
      );
    ...
    it('should throw error for non-existent user', async () => {
      await expect(
        favoritesService.areFavorites('non-existent-user', [testRecipe._id.toString()])
      ).rejects.toThrow(TRPCError);
    });
  });
    ...
    it('should handle concurrent add operations correctly', async () => {
      ...
      const promises = Array(5).fill(null).map(async (_, index) => {
        try {
          ...
          return await favoritesService.addToFavorites(context, recipe._id.toString());
        } catch (error) {
          return error;
        }
      });
  ...
  describe('error handling', () => {
    it('should handle database errors gracefully', async () => {
      // Mock database error
      const mockFindOne = vi.spyOn(DindinUser, 'findOne')
    ...
    it('should handle validation errors', async () => {
      ...
      const mockFindByIdAndUpdate = vi.spyOn(DindinUser, 'findByIdAndUpdate')
