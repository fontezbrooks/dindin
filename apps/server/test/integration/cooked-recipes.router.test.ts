...
describe('Cooked Recipes Router Integration', () => {
  ...
  describe('markAsCooked', () => {
    ...
    it('should create partner session when cooking with partner', async () => {
      ...
      const partnerRecord = await CookedRecipe.findOne({ userId: partnerUser._id });
  ...
  describe('getRecentlyCooked', () => {
    ...
    it('should get recently cooked recipes', async () => {
      ...
      expect(new Date(result.cookedRecipes[0].cookedAt).getTime())
    ...
    it('should filter by date range', async () => {
      ...
      expect(result.cookedRecipes[0].notes).toBe('Even better the second time!');
    });
    ...
    it('should filter by minimum rating', async () => {
      ...
      expect(result.cookedRecipes.every(recipe => recipe.rating! >= 4)).toBe(true);
    });
