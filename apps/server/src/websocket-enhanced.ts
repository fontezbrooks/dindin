...
export class EnhancedWebSocketServer extends EventEmitter {
	...
	constructor(port: number = 3001) {
		...
		this.wss = new WebSocketServer({
			...
			verifyClient: async (info, cb) => {
				try {
					...
					if (!token) {
						logger.log(
							"WebSocket connection allowed without token (for auth operations)",
						);
						cb(true);
						return;
					}
					...
					if (!session) {
						logger.log("WebSocket connection rejected: Invalid token");
						cb(false, 401, "Invalid token");
						return;
					}

					logger.log(
						"WebSocket connection authenticated for session:",
						session?.user?.id,
					);
					cb(true);
				} catch (error) {
		...
		logger.log(
			`✅ Enhanced WebSocket Server listening on ws://localhost:${port}`,
		);
	}
	...
	private setupHandlers() {
		...
		this.wss.on("connection", async (ws: WSClient, req) => {
			logger.log(`➕ WebSocket connection attempt`);
			...
			try {
				...
				if (session?.user) {
					...
					if (user) {
						...
						logger.log(
							`✅ User ${user.name} (${ws.userId}) connected via WebSocket`,
						);
						...
						if (ws.partnerId && this.clients.has(ws.partnerId)) {
							...
							logger.log(
								`📢 Notified partner ${ws.partnerId} that user is online`,
							);
						}
						...
						logger.log(
							"⚠️ DindinUser not found for auth user:",
							session.user.id,
						);
						...
						ws.send(
							JSON.stringify({
								type: "connected",
								message:
									"WebSocket connection established (pending user creation)",
							}),
						);
					}
				} else {
					// Send connection message for unauthenticated connections
					logger.log("ℹ️ Unauthenticated WebSocket connection");
					ws.send(
						JSON.stringify({
							type: "connected",
			...
			ws.on("close", () => {
				...
				if (ws.userId) {
					...
					if (ws.partnerId && this.clients.has(ws.partnerId)) {
						...
						logger.log(
							`📢 Notified partner ${ws.partnerId} that user is offline`,
						);
					}
				}
			});
...
export function createEnhancedWebSocketServer(
	port?: number,
