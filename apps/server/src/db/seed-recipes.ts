...
async function seedRecipes() {
	try {
		...
		logger.log("Cleared existing recipes");

		// Insert sample recipes
		const recipes = await Recipe.insertMany(sampleRecipes);
		logger.log(`✅ Successfully added ${recipes.length} sample recipes`);

		// Display the recipes
		recipes.forEach((recipe) => {
			logger.log(`- ${recipe.title} (${recipe.cuisine}, ${recipe.difficulty})`);
		});
	} catch (error) {
