  ...
  browseAllRecipes: publicProcedure
    .input(
      z.object({
        ...
        sortBy: z.enum(["popularity", "rating", "newest", "cookTime"]).default("popularity"),
        limit: z.number().min(1).max(50).default(20),
        offset: z.number().min(0).default(0),
      })
    )
  ...
  getUserFavorites: protectedProcedure
    .input(
      z.object({
        limit: z.number().min(1).max(50).default(20),
        offset: z.number().min(0).default(0),
        sortBy: z.enum(['newest', 'oldest', 'alphabetical', 'cookTime']).optional(),
        filterBy: z.object({
          cuisine: z.string().optional(),
          difficulty: z.enum(['easy', 'medium', 'hard']).optional(),
          maxCookTime: z.number().optional(),
    ...
    .query(async ({ input, ctx }) => {
      ...
      if (input.filterBy) {
        if (input.filterBy.cuisine) {
          favorites = favorites.filter(recipe => recipe.cuisine === input.filterBy.cuisine);
        }
        if (input.filterBy.difficulty) {
          favorites = favorites.filter(recipe => recipe.difficulty === input.filterBy.difficulty);
        }
        if (input.filterBy.maxCookTime) {
          favorites = favorites.filter(recipe => recipe.cook_time <= input.filterBy.maxCookTime);
        }
