...
import { z } from "zod";
import { CookedRecipe, DindinUser, Recipe } from "../db";
...
export const cookedRecipesRouter = router({
	...
	getCookingStats: protectedProcedure.query(async ({ ctx }) => {
		// Get user profile
		...
		const mostCooked = await CookedRecipe.getMostCookedRecipes(
			user._id.toString(),
			5,
		);
		...
		const startDate = cookingStreak.find(
			(day) => day._id === today || day._id === yesterday,
		)?._id;
		if (startDate) {
	...
	deleteCookingSession: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			user.cookedRecipes = user.cookedRecipes.filter(
				(cooked: any) =>
					!cooked.recipeId.equals(session.recipeId) ||
