...
const addCustomItemSchema = z.object({
  ...
  substituteOptions: z.array(z.object({
    name: z.string(),
...
router.get('/', async (req, res) => {
  try {
    ...
    const shoppingLists = await shoppingListService.getUserShoppingLists(userId, options);
...
router.post('/', validateRequest(createShoppingListSchema), async (req, res) => {
  try {
    const userId = req.user.id;
    const input = {
      ...req.body,
      ...
      scheduledShoppingDate: req.body.scheduledShoppingDate ? new Date(req.body.scheduledShoppingDate) : undefined,
    };
    
    const shoppingList = await shoppingListService.createShoppingList(userId, input);
...
router.get('/analytics', async (req, res) => {
  try {
    ...
    const analytics = await shoppingListService.getShoppingAnalytics(userId, dateRange);
    res.json(analytics);
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
...
router.put('/:id/settings', validateRequest(updateSettingsSchema), async (req, res) => {
  try {
    const userId = req.user.id;
    const { id } = req.params;
    const settings = {
    ...
    const updatedList = await shoppingListService.updateShoppingListSettings(userId, id, settings);
...
router.post('/:id/items', validateRequest(addCustomItemSchema), async (req, res) => {
  try {
    const userId = req.user.id;
    const { id } = req.params;
    
    const updatedList = await shoppingListService.addCustomItem(userId, id, req.body);
    res.json(updatedList);
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
...
router.put('/:id/items/:itemIndex', validateRequest(updateItemSchema), async (req, res) => {
  try {
    const userId = req.user.id;
    ...
    const updatedList = await shoppingListService.updateShoppingItem(userId, id, index, req.body);
    res.json(updatedList);
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
...
router.delete('/:id/items/:itemIndex', async (req, res) => {
  try {
    ...
    const updatedList = await shoppingListService.removeShoppingItem(userId, id, index);
    res.json(updatedList);
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
...
router.post('/:id/items/:itemIndex/purchase', validateRequest(purchaseItemSchema), async (req, res) => {
  try {
    const userId = req.user.id;
    const { id, itemIndex } = req.params;
    const index = parseInt(itemIndex);
    ...
    const updatedList = await shoppingListService.purchaseItem(userId, id, index, actualPrice, store);
    res.json(updatedList);
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
...
router.post('/:id/optimize-route', async (req, res) => {
  try {
    ...
    const optimizedRoute = await shoppingListService.optimizeShoppingRoute(userId, id);
...
router.post('/:id/share', validateRequest(shareShoppingListSchema), async (req, res) => {
  try {
    const userId = req.user.id;
    const { id } = req.params;
    const { shareWithUserId, permission, canPurchase } = req.body;
    
    const sharedList = await shoppingListService.shareShoppingList(userId, id, shareWithUserId, permission, canPurchase);
    res.json(sharedList);
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
...
router.post('/:id/complete-session', validateRequest(completeSessionSchema), async (req, res) => {
  try {
    const userId = req.user.id;
    const { id } = req.params;
    
    const updatedList = await shoppingListService.completeShoppingSesssion(userId, id, req.body);
    res.json(updatedList);
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
...
router.post('/templates/:templateId/duplicate', async (req, res) => {
  try {
    ...
    const customizations = {
      ...
      scheduledShoppingDate: scheduledShoppingDate ? new Date(scheduledShoppingDate) : undefined,
    };
    
    const duplicatedList = await shoppingListService.duplicateFromTemplate(userId, templateId, customizations);
...
router.use((error: any, req: any, res: any, next: any) => {
  console.error('Shopping List Router Error:', error);
