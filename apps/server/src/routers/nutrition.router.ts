...
nutrition.get('/recipe/:recipeId', async (c) => {
  try {
    ...
    const nutritionData = await NutritionCalculationService.getNutritionForServings(recipeId, servings);
    ...
    const macroPercentages = NutritionCalculationService.calculateMacroPercentages(nutritionData);
    const dailyValuePercentages = Nutrition.getDailyValuePercentages(nutritionData);
...
nutrition.get('/compare/:recipeId1/:recipeId2', async (c) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(recipeId1) || !mongoose.Types.ObjectId.isValid(recipeId2)) {
    ...
    if (!nutrition1 || !nutrition2) {
      return c.json({ error: 'Nutrition data not found for one or both recipes' }, 404);
    }
    
    const comparison = Nutrition.compareNutrition(nutrition1, nutrition2);
    const macros1 = NutritionCalculationService.calculateMacroPercentages(nutrition1);
    const macros2 = NutritionCalculationService.calculateMacroPercentages(nutrition2);
...
nutrition.get('/daily/:userId', async (c) => {
  try {
    ...
    const dailyNutrition = await DailyNutritionService.getDailyNutrition(userId, date);
    const recommendations = await DailyNutritionService.getNutritionRecommendations(userId, date);
...
nutrition.delete('/daily/:userId/meal/:mealId', async (c) => {
  try {
    ...
    const updatedDailyNutrition = await DailyNutritionService.removeMeal(userId, mealId, date);
...
nutrition.post('/daily/:userId/water', async (c) => {
  try {
    ...
    const updatedDailyNutrition = await DailyNutritionService.addWaterIntake(userId, ounces, date);
...
nutrition.put('/daily/:userId/goals', async (c) => {
  try {
    ...
    const updatedDailyNutrition = await DailyNutritionService.updateGoals(userId, validatedGoals);
...
nutrition.get('/history/:userId', async (c) => {
  try {
    ...
    const history = await DailyNutritionService.getNutritionHistory(userId, startDate, endDate);
...
nutrition.get('/weekly/:userId', async (c) => {
  try {
    ...
    const weeklySummary = await DailyNutritionService.getWeeklySummary(userId, startDate);
...
nutrition.get('/analytics/:userId', async (c) => {
  try {
    ...
    const analytics = await DailyNutritionService.getNutritionAnalytics(userId, days);
...
nutrition.get('/suggestions/:userId', async (c) => {
  try {
    ...
    const suggestions = await DailyNutritionService.getMealSuggestions(userId, mealType, date);
...
nutrition.post('/daily/:userId/complete', async (c) => {
  try {
    ...
    const dailyNutrition = await DailyNutritionService.markDayCompleted(userId, date);
...
nutrition.post('/recipe/:recipeId/nutrition', async (c) => {
  try {
    ...
    const nutritionData = await NutritionCalculationService.updateRecipeNutrition(
      recipeId,
      ingredients,
      servings || 1
    );
