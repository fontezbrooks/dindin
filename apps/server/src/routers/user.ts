...
export const userRouter = router({
	...
	removeFromFavorites: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			try {
				...
				logger.error("Error removing recipe from favorites:", error);
				...
				throw new TRPCError({
					code: "INTERNAL_SERVER_ERROR",
					message:
						"An unexpected error occurred while removing recipe from favorites",
				});
			}
		}),
	...
	addToFavorites: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			try {
				...
				// Log error for debugging but don't expose internal details
				logger.error("Error adding recipe to favorites:", error);
				...
				throw new TRPCError({
					code: "INTERNAL_SERVER_ERROR",
					message:
						"An unexpected error occurred while adding recipe to favorites",
				});
			}
		}),
	...
	isFavorite: protectedProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			const recipeObjectId = new mongoose.Types.ObjectId(input.recipeId);
			const isFavorite =
				user.likedRecipes?.some((likedId) => likedId.equals(recipeObjectId)) ||
				false;
