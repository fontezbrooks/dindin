...
const assignMealSchema = z.object({
  mealPlanId: z.string(),
...
router.get('/', async (req, res) => {
  try {
    ...
    const mealPlans = await mealPlanningService.getUserMealPlans(userId, options);
    res.json(mealPlans);
  } catch (error) {
...
router.put('/:id', validateRequest(updateMealPlanSchema), async (req, res) => {
  try {
    ...
    const updatedPlan = await mealPlanningService.updateMealPlan(userId, id, req.body);
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    await mealPlanningService.updateMealPlan(userId, id, { status: 'archived' });
    res.status(204).send();
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
...
router.post('/:id/share', validateRequest(shareMealPlanSchema), async (req, res) => {
  try {
    const userId = req.user.id;
    const { id } = req.params;
    const { shareWithUserId, permission } = req.body;
    
    const sharedPlan = await mealPlanningService.shareMealPlan(userId, id, shareWithUserId, permission);
    res.json(sharedPlan);
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
...
router.post('/meals/assign', validateRequest(assignMealSchema), async (req, res) => {
  try {
    const userId = req.user.id;
    const input = {
      ...req.body,
...
router.post('/meals/complete', validateRequest(completeMealSchema), async (req, res) => {
  try {
    const userId = req.user.id;
    const { mealPlanId, date, mealType, rating, notes, modifications } = req.body;
...
router.post('/:id/prep-tasks', validateRequest(addPrepTaskSchema), async (req, res) => {
  try {
    const userId = req.user.id;
    const { id } = req.params;
    const taskData = {
    ...
    const updatedPlan = await mealPlanningService.addMealPrepTask(userId, id, taskData.date, taskData);
    res.json(updatedPlan);
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
...
router.get('/:id/shopping-list', async (req, res) => {
  try {
    ...
    const options = {
      ...
      excludeCategories: excludeCategories ? (excludeCategories as string).split(',') : [],
    };
    
    const shoppingList = await mealPlanningService.generateShoppingList(userId, id, options);
...
router.get('/:id/nutrition', async (req, res) => {
  try {
    ...
    const nutritionalOverview = await mealPlanningService.calculateNutritionalOverview(id);
...
router.use((error: any, req: any, res: any, next: any) => {
  console.error('Meal Planning Router Error:', error);
