...
export class DailyNutritionService {
  ...
  static async addMeal(userId: string, mealType: string, recipeId: string, servings: number = 1, date?: Date) {
    try {
      const dailyNutrition = await this.getDailyNutrition(userId, date);
      
      // Get nutrition data for the recipe
      const recipeNutrition = await NutritionCalculationService.getNutritionForServings(recipeId, servings);
      
      // Add meal to daily nutrition
      await dailyNutrition.addMeal(mealType, recipeId, servings, recipeNutrition);
      
      // Return updated daily nutrition with populated meals
      return await this.getDailyNutrition(userId, date);
    } catch (error) {
  ...
  static async removeMeal(userId: string, mealId: string, date?: Date) {
    try {
      ...
      console.error('Error removing meal from daily nutrition:', error);
  ...
  static async updateGoals(userId: string, goals: any) {
    try {
      ...
      const dailyNutrition = await this.getDailyNutrition(userId, today);
  ...
  static async getNutritionHistory(userId: string, startDate: Date, endDate: Date) {
    try {
      ...
      }).sort({ date: -1 }).populate('meals.recipeId', 'title image_url');
    } catch (error) {
  ...
  static async getNutritionStreak(userId: string) {
    try {
      ...
      console.error('Error fetching nutrition streak:', error);
    ...
    try {
      ...
      if (goalPercentages.calories < 50) {
        recommendations.push({
          ...
          priority: 'medium',
          suggestions: ['nuts', 'fruit with nut butter', 'whole grain crackers']
        });
      ...
      if (goalPercentages.protein < 80) {
        recommendations.push({
          ...
          priority: 'high',
          suggestions: ['lean meats', 'eggs', 'legumes', 'Greek yogurt', 'tofu']
        });
      }
      ...
      if (goalPercentages.fiber < 70) {
        recommendations.push({
          ...
          priority: 'medium',
          suggestions: ['whole grains', 'beans', 'vegetables', 'fruits with skin']
        });
      }
      ...
      if (goalPercentages.sodium > 90) {
        recommendations.push({
          ...
          priority: 'high',
          suggestions: ['fresh fruits', 'unsalted nuts', 'herbs instead of salt']
        });
      }
      ...
      const macros = dailyNutrition.macroPercentages || { protein: 0, carbs: 0, fat: 0 };
      if (macros.fat > 40) {
        recommendations.push({
          type: 'macros',
          message: 'Your fat intake is high. Balance with more proteins and complex carbs.',
          priority: 'medium',
  ...
  static async getMealSuggestions(userId: string, mealType: string, date?: Date) {
    try {
      const dailyNutrition = await this.getDailyNutrition(userId, date);
      const remaining = dailyNutrition.remaining;
      const goalPercentages = dailyNutrition.goalPercentages;
      ...
      const suggestions = {
        targetCalories: Math.min(remaining.calories, this.getMealCalorieTarget(mealType)),
  ...
  static async getNutritionAnalytics(userId: string, days: number = 30) {
    try {
      ...
      const totals = records.reduce((acc, record) => {
        ...
        return acc;
      }, { calories: 0, protein: 0, carbohydrates: 0, fat: 0, fiber: 0, water: 0 });
      ...
      if (firstRecord && firstRecord.goals) {
        Object.keys(firstRecord.goals).forEach(nutrient => {
          const achievedDays = records.filter(record => {
            const actual = record[`total${nutrient.charAt(0).toUpperCase() + nutrient.slice(1)}`] || record.totalCalories;
            const goal = record.goals[nutrient];
            return actual >= goal * 0.8; // 80% of goal counts as achieved
          }).length;
          
          goalAchievement[nutrient] = Math.round((achievedDays / records.length) * 100);
        });
      }
      
      // Generate insights
      const insights = this.generateNutritionInsights(records, averages, goalAchievement);
      ...
      return {
        ...
        goalAchievement,
        insights,
        streak: await this.getNutritionStreak(userId)
      };
    } catch (error) {
      console.error('Error generating nutrition analytics:', error);
      throw new Error('Failed to generate nutrition analytics');
    }
  }

  // Generate nutrition insights
  private static generateNutritionInsights(records: any[], averages: any, goalAchievement: any) {
    const insights = [];
    
    // Calorie consistency insight
    const calorieVariance = this.calculateVariance(records.map(r => r.totalCalories));
    if (calorieVariance < 200) {
      insights.push({
        type: 'positive',
        title: 'Consistent Calorie Intake',
        message: 'You maintain consistent calorie intake, which is great for metabolic health.'
      });
    } else {
      insights.push({
        type: 'suggestion',
        title: 'Calorie Consistency',
        message: 'Try to maintain more consistent daily calorie intake for better results.'
      });
    }
    ...
    if (averages.water < 48) {
      insights.push({
        ...
        message: 'Your average water intake is below recommended levels. Aim for 8 glasses per day.'
      });
    }
    
    // Weekend vs weekday patterns
    const weekdays = records.filter(r => [1, 2, 3, 4, 5].includes(r.date.getDay()));
    const weekends = records.filter(r => [0, 6].includes(r.date.getDay()));
    
    if (weekdays.length > 0 && weekends.length > 0) {
      const weekdayAvgCals = weekdays.reduce((sum, r) => sum + r.totalCalories, 0) / weekdays.length;
      const weekendAvgCals = weekends.reduce((sum, r) => sum + r.totalCalories, 0) / weekends.length;
      
      if (weekendAvgCals > weekdayAvgCals * 1.2) {
        insights.push({
          type: 'suggestion',
          title: 'Weekend Eating Pattern',
          message: 'You tend to eat more on weekends. Try to maintain consistency throughout the week.'
        });
      }
    }
