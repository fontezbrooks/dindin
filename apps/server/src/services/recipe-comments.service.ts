...
export class RecipeCommentsService {
	...
	static async addComment(
		...
		const comment = new RecipeComment({
			...
			sharedRecipeId: sharedRecipeId
				? new mongoose.Types.ObjectId(sharedRecipeId)
				: undefined,
		...
		if (parentCommentId) {
			await RecipeComment.findByIdAndUpdate(parentCommentId, {
				$inc: { replies: 1 },
			});

			// Notify parent comment author
			const parentComment = await RecipeComment.findById(
				parentCommentId,
		...
		return RecipeComment.findById(comment._id).populate(
			"userId",
		...
		if (userId) {
			replies = replies.map((reply) => {
				...
				return {
					...
					userVote: userVote?.vote || null,
					netHelpfulVotes:
						reply.isHelpful.helpfulVotes - reply.isHelpful.notHelpfulVotes,
				};
			});
		}
	...
	static async voteOnComment(
		...
		return RecipeComment.findById(commentId).populate(
			"userId",
			"name username avatar",
		);
	}
	...
	static async deleteComment(
		...
		if (comment.parentCommentId) {
			await RecipeComment.findByIdAndUpdate(comment.parentCommentId, {
				$inc: { replies: -1 },
			});
		}
	}
	...
	static async getCommentStats(recipeId: string): Promise<{
		...
		const commentsByType = result.commentsByType.reduce(
			(acc: any, type: string) => {
