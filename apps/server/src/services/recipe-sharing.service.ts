...
export class RecipeSharingService {
  private static readonly BASE_URL = process.env.BASE_URL || 'https://dindin.app';
  ...
  static async createShareableLink(
    ...
    let existingShare = await SharedRecipe.findOne({
    ...
    const shareUrl = `${this.BASE_URL}/recipe/shared/${shareToken}`;
    ...
    if (shareType === 'specific_users' && sharedWith.length > 0) {
      await this.notifyUsersOfSharedRecipe(sharedRecipe, sharedWith);
    }
  ...
  static async getSharedRecipe(
    ...
    const sharedRecipe = await SharedRecipe.findOne({
      ...
      {
        path: 'recipeId',
        select: 'title description ingredients instructions image_url cook_time prep_time servings difficulty cuisine tags',
      },
      {
        path: 'sharedBy',
    ...
    if (!this.canViewSharedRecipe(sharedRecipe, viewerUserId)) {
  ...
  private static canViewSharedRecipe(
    ...
    switch (sharedRecipe.shareType) {
      ...
      case 'specific_users':
        return viewerUserId && sharedRecipe.sharedWith.some(
   ...
   */
  static generateSocialSharingUrls(shareUrl: string, recipe: any): {
    whatsapp: string;
    facebook: string;
    twitter: string;
    email: string;
  ...
  static async shareWithUsers(
    ...
    const { shareUrl, shareToken } = await this.createShareableLink(
      recipeId,
      fromUserId,
      {
   ...
   */
  static generateOGMetadata(recipe: any, sharedBy: any): {
    [key: string]: string;
  } {
    return {
      'og:title': recipe.title,
      'og:description': recipe.description || `A delicious ${recipe.cuisine} recipe shared by ${sharedBy.name}`,
      'og:image': recipe.image_url || '/default-recipe-image.jpg',
      'og:type': 'article',
      'og:site_name': 'DinDin Recipe App',
      'twitter:card': 'summary_large_image',
      'twitter:title': recipe.title,
      'twitter:description': recipe.description || `A delicious ${recipe.cuisine} recipe`,
  ...
  static async updateSharePrivacy(
    ...
    if (!sharedRecipe) {
      throw new Error('Shared recipe not found or you do not have permission to modify it');
    }
  ...
  static async deactivateShare(
    ...
    if (!sharedRecipe) {
      throw new Error('Shared recipe not found or you do not have permission to deactivate it');
    }
