...
export class MatchService {
	...
	static async createMatch(
		...
		try {
			logger.log(
				`Creating match between users ${userId1} and ${userId2} for recipe ${recipeId}`,
			);
			...
			if (existingMatch) {
				logger.log("Match already exists");
			...
			if (wsServer) {
				...
				if (notificationsSent) {
					logger.log(`✅ Match notifications sent to both partners`);
				} else {
					logger.log(
						"⚠️ One or both partners not connected for real-time notification",
					);
				}
			} else {
				logger.log("⚠️ WebSocket server not available");
			}
			...
			await MatchService.sendPushNotifications(users, match);

			logger.log(`✅ Match created successfully: ${match._id}`);
			return match;
		} catch (error) {
	...
	static async sendPushNotifications(users: any[], match: any) {
		...
		logger.log(
			"Push notifications would be sent here to:",
	...
	static async checkForMatch(
		...
		try {
			...
			if (!partner) {
				logger.log("Partner not found");
				return false;
			}

			// Check if partner has liked this recipe
			const hasLiked =
				partner.likedRecipes?.some(
					(id) => id.toString() === recipeId.toString(),
				) || false;

			logger.log(
				`Partner ${partner.name} has ${hasLiked ? "" : "NOT "}liked recipe ${recipeId}`,
			);
			return hasLiked;
		} catch (error) {
	...
	static async notifyPartnerActivity(
		...
		if (!wsServer) {
			logger.log(
				"WebSocket server not available for partner activity notification",
			);
			return false;
		}
