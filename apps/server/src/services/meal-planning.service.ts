...
export class MealPlanningService {
	...
	async createMealPlan(userId: string, input: CreateMealPlanInput) {
		try {
			...
			if (input.templateId) {
				const duplicatedPlan = await MealPlan.duplicatePlan(
					input.templateId,
					userId,
					input.startDate,
				);
				...
				if (input.budgetSettings)
					duplicatedPlan.budgetSettings = input.budgetSettings;
				if (input.notifications)
					duplicatedPlan.notifications = input.notifications;
		...
		try {
			...
			const mealAssignment = {
				...
				recipeId: input.recipeId
					? new mongoose.Types.ObjectId(input.recipeId)
					: null,
	...
	async removeMeal(
		...
		try {
			...
			// Update analytics
			mealPlan.analytics.totalMealsPlanned = Math.max(
				0,
				mealPlan.analytics.totalMealsPlanned - 1,
			);
		...
		try {
			...
			mealPlan.analytics.completionRate = Math.round(
				(mealPlan.analytics.totalMealsCompleted /
					mealPlan.analytics.totalMealsPlanned) *
					100,
			);
	...
	async shareMealPlan(
		...
		try {
			...
			const existingShare = mealPlan.sharedWith.find(
	...
	async calculateNutritionalOverview(mealPlanId: string) {
		try {
					...
					if (meal.recipeId && meal.recipeId.nutrition) {
						...
						if (recipe.nutrition) {
							dayNutrition.calories +=
								(recipe.nutrition.calories || 0) * servingMultiplier;
							dayNutrition.protein +=
								(recipe.nutrition.protein || 0) * servingMultiplier;
							dayNutrition.carbs +=
								(recipe.nutrition.carbs || 0) * servingMultiplier;
							dayNutrition.fat +=
								(recipe.nutrition.fat || 0) * servingMultiplier;
							dayNutrition.fiber +=
								(recipe.nutrition.fiber || 0) * servingMultiplier;
							dayNutrition.sugar +=
								(recipe.nutrition.sugar || 0) * servingMultiplier;
						}
					}
				}
			...
			if (nutritionalOverview.totalDays > 0) {
				nutritionalOverview.averageDaily.calories = Math.round(
					nutritionalOverview.weeklyTotals.calories /
						nutritionalOverview.totalDays,
				);
				nutritionalOverview.averageDaily.protein = Math.round(
					nutritionalOverview.weeklyTotals.protein /
						nutritionalOverview.totalDays,
				);
				nutritionalOverview.averageDaily.carbs = Math.round(
					nutritionalOverview.weeklyTotals.carbs /
						nutritionalOverview.totalDays,
				);
				nutritionalOverview.averageDaily.fat = Math.round(
					nutritionalOverview.weeklyTotals.fat / nutritionalOverview.totalDays,
				);
				nutritionalOverview.averageDaily.fiber = Math.round(
					nutritionalOverview.weeklyTotals.fiber /
						nutritionalOverview.totalDays,
				);
				nutritionalOverview.averageDaily.sugar = Math.round(
					nutritionalOverview.weeklyTotals.sugar /
						nutritionalOverview.totalDays,
				);
			}
			...
			throw new Error(
				`Failed to calculate nutritional overview: ${error.message}`,
			);
		}
	}
}
