...
export class CookingAnalyticsService {
	 ...
	 */
	static async generateCookingInsights(
		userId: string,
		...
		if (cookingSessions.length === 0) {
			return CookingAnalyticsService.getEmptyInsights();
		}

		// Calculate favorite time of day
		const timeSlots =
			CookingAnalyticsService.categorizeByTimeOfDay(cookingSessions);
		const favoriteTimeOfDay =
			CookingAnalyticsService.getMostFrequent(timeSlots);
		...
		const dayOfWeekCounts =
			CookingAnalyticsService.categorizeByDayOfWeek(cookingSessions);
		const mostProductiveDay =
			CookingAnalyticsService.getMostFrequent(dayOfWeekCounts);

		// Calculate current cooking streak
		const cookingStreak =
			CookingAnalyticsService.calculateCookingStreak(cookingSessions);

		// Identify improvement areas
		const improvementAreas =
			CookingAnalyticsService.identifyImprovementAreas(cookingSessions);

		// Calculate personal bests
		const personalBests = await CookingAnalyticsService.calculatePersonalBests(
			userObjectId,
			cookingSessions,
		);
	 ...
	 */
	static async generateCookingTrends(
		userId: string,
		months: number = 6,
	): Promise<CookingTrends> {
		const userObjectId = new mongoose.Types.ObjectId(userId);
		...
		const weeklyActivity =
			CookingAnalyticsService.analyzeWeeklyActivity(cookingSessions);

		// Monthly activity analysis
		const monthlyActivity = CookingAnalyticsService.analyzeMonthlyActivity(
			cookingSessions,
			months,
		);
		...
		const ratingTrends =
			CookingAnalyticsService.analyzeRatingTrends(cookingSessions);

		// Cuisine preferences with ratings
		const cuisinePreferences =
			await CookingAnalyticsService.analyzeCuisinePreferences(cookingSessions);

		// Difficulty progression over time
		const difficultyProgression =
			CookingAnalyticsService.analyzeDifficultyProgression(cookingSessions);
	...
	static async getPersonalizedRecommendations(userId: string) {
		...
		if (!user || cookingSessions.length === 0) {
			return CookingAnalyticsService.getDefaultRecommendations();
		}

		// Analyze preferences
		const preferredCuisines =
			CookingAnalyticsService.extractPreferredCuisines(cookingSessions);
		const preferredDifficulty =
			CookingAnalyticsService.extractPreferredDifficulty(cookingSessions);
		const averageRating =
			cookingSessions
				.filter((s) => s.rating)
	...
	static async calculateCookingGoals(userId: string) {
		...
		const goals = {
			totalRecipes: {
				...
				nextTarget: CookingAnalyticsService.getNextTarget(
			...
			cuisineExplorer: {
				current: CookingAnalyticsService.countUniqueCuisines(cookingSessions),
				targets: [3, 5, 10, 15, 20],
				nextTarget: CookingAnalyticsService.getNextTarget(
					CookingAnalyticsService.countUniqueCuisines(cookingSessions),
					[3, 5, 10, 15, 20],
				),
			},
			masterChef: {
				current: cookingSessions.filter((s) => s.rating && s.rating >= 5)
					.length,
	...
	private static categorizeByTimeOfDay(
		sessions: any[],
	...
	private static categorizeByDayOfWeek(
		sessions: any[],
	): Record<string, number> {
		const days = [
			"Sunday",
	...
	private static getMostFrequent(counts: Record<string, number>): string {
		return Object.entries(counts).reduce(
			(max, [key, value]) => (value > counts[max] ? key : max),
	...
	private static identifyImprovementAreas(sessions: any[]): string[] {
		...
		if (sessions.length > 0) {
			const daysSinceFirst = Math.floor(
				(Date.now() -
					new Date(sessions[sessions.length - 1].cookedAt).getTime()) /
					(1000 * 60 * 60 * 24),
			);
			const frequency = sessions.length / Math.max(daysSinceFirst, 1);
			if (frequency < 0.2) {
				// Less than once every 5 days
	...
	private static async calculatePersonalBests(
		userId: mongoose.Types.ObjectId,
		sessions: any[],
	) {
		// Longest streak calculation (simplified)
		const longestStreak =
			CookingAnalyticsService.calculateCookingStreak(sessions);
	...
	private static analyzeMonthlyActivity(sessions: any[], months: number) {
		...
		for (let i = months - 1; i >= 0; i--) {
			...
			const count = sessions.filter((session) => {
				...
				return (
					sessionMonth.getMonth() === month.getMonth() &&
	...
	private static async analyzeCuisinePreferences(sessions: any[]) {
		...
		return Array.from(cuisineData.entries())
			.map(([cuisine, data]) => ({
				cuisine,
				count: data.count,
				avgRating:
		...
		sessions.forEach((session) => {
			if (session.recipeId?.difficulty) {
				...
				const difficultyValue =
					difficultyValues[session.recipeId.difficulty.toLowerCase()] || 1;
