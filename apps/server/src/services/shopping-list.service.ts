...
export class ShoppingListService {
	...
	async getUserShoppingLists(
		...
		try {
			...
			if (options.mealPlanId)
				query.mealPlanId = new mongoose.Types.ObjectId(options.mealPlanId);
		...
		try {
			...
			if (updates.isPurchased && !item.isPurchased) {
				...
				if (updates.actualPrice !== undefined) {
					shoppingList.budget.actual =
						(shoppingList.budget.actual || 0) + updates.actualPrice;
				}
			}
	...
	async purchaseItem(
		...
		try {
			...
			if (
				shoppingList.shoppingSessions.length === 0 ||
				shoppingList.shoppingSessions[shoppingList.shoppingSessions.length - 1]
					.completedAt
			) {
				...
				const currentSession =
					shoppingList.shoppingSessions[
						shoppingList.shoppingSessions.length - 1
					];
	 ...
	 */
	async completeShoppingSesssion(
		userId: string,
		shoppingListId: string,
		sessionData?: {
		...
		try {
			...
			if (sessionData?.paymentMethod)
				currentSession.paymentMethod = sessionData.paymentMethod;
	...
	async optimizeShoppingRoute(userId: string, shoppingListId: string) {
		try {
			...
			if (!optimized) {
				throw new Error(
					"Unable to optimize route - no preferred stores configured",
				);
			}
	...
	async shareShoppingList(
		...
		try {
			...
			const existingShare = shoppingList.sharedWith.find(
	...
	async getTemplates(category?: string, limit = 20) {
		try {
			...
			throw new Error(
				`Failed to get shopping list templates: ${error.message}`,
			);
		}
	}
	...
	async duplicateFromTemplate(
		...
		try {
			...
			if (customizations?.dateRange)
				duplicateData.dateRange = customizations.dateRange;
			if (customizations?.scheduledShoppingDate)
				duplicateData.scheduledShoppingDate =
					customizations.scheduledShoppingDate;
	...
	async updateShoppingListSettings(
		...
		try {
			...
			if (settings.preferredStores)
				shoppingList.preferredStores = settings.preferredStores;
			if (settings.budget) Object.assign(shoppingList.budget, settings.budget);
			if (settings.scheduledShoppingDate)
				shoppingList.scheduledShoppingDate = settings.scheduledShoppingDate;
			if (settings.reminderSettings)
				shoppingList.reminderSettings = settings.reminderSettings;
			if (settings.smartFeatures)
				Object.assign(shoppingList.smartFeatures, settings.smartFeatures);

			await shoppingList.save();
			return shoppingList;
		} catch (error) {
			throw new Error(
				`Failed to update shopping list settings: ${error.message}`,
			);
		}
	}
}
