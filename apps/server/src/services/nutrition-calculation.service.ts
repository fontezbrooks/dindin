...
export class NutritionCalculationService {
	...
	static async calculateFromIngredients(
		ingredients: any[],
		servings: number = 1,
	) {
		...
		const ingredientNutrition =
			NutritionCalculationService.getIngredientNutritionDatabase();

		for (const ingredient of ingredients) {
			const nutrition = NutritionCalculationService.estimateIngredientNutrition(
				ingredient,
				ingredientNutrition,
			);
		...
		Object.keys(totalNutrition).forEach((key) => {
			totalNutrition[key] =
				Math.round((totalNutrition[key] / servings) * 10) / 10;
		});
	...
	private static estimateIngredientNutrition(ingredient: any, database: any) {
		...
		if (database[name]) {
			...
			const partialMatch = Object.keys(database).find(
		...
		const servingMultiplier =
			NutritionCalculationService.convertToServingMultiplier(amount, unit);
	...
	private static convertToServingMultiplier(
		amount: number,
		unit: string,
	): number {
		const conversions = {
	...
	static determineDietaryTags(ingredients: any[], nutrition: any) {
		...
		if (!animalProducts.some((product) => ingredientNames.includes(product))) {
			...
			!["chicken", "beef", "pork", "fish", "salmon", "tuna"].some((meat) =>
	...
	static detectAllergens(ingredients: any[]) {
		...
		Object.keys(allergenMap).forEach((allergen) => {
			if (
				allergenMap[allergen].some((trigger) =>
					ingredientNames.includes(trigger),
				)
			) {
	...
	static async createNutritionProfile(
		recipeId: string,
		ingredients: any[],
		servings: number = 1,
	) {
		const calculatedNutrition =
			await NutritionCalculationService.calculateFromIngredients(
				ingredients,
				servings,
			);
		const dietaryTags = NutritionCalculationService.determineDietaryTags(
			ingredients,
			calculatedNutrition,
		);
		const allergens = NutritionCalculationService.detectAllergens(ingredients);
	...
	static async updateRecipeNutrition(
		...
		const nutritionProfile =
			await NutritionCalculationService.createNutritionProfile(
				recipeId,
				ingredients,
				servings,
			);
	...
	static async getNutritionForServings(
		recipeId: string,
