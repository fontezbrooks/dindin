...
mealPlanSchema.virtual("progressPercentage").get(function () {
	...
	return Math.round(
		(this.analytics.totalMealsCompleted / this.analytics.totalMealsPlanned) *
			100,
	);
});
...
mealPlanSchema.methods.calculateDayNutrition = (dayPlan: any) => {
...
mealPlanSchema.statics.getTemplates = async function (
	category?: string,
	limit = 20,
) {
...
mealPlanSchema.statics.duplicatePlan = async function (
	...
	planData.endDate = new Date(
		startDate.getTime() + originalDuration * 24 * 60 * 60 * 1000,
	);
