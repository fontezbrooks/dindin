...
dailyNutritionSchema.virtual('goalPercentages').get(function() {
  return {
    ...
    carbohydrates: Math.round((this.totalCarbohydrates / this.goals.carbohydrates) * 100),
...
dailyNutritionSchema.virtual('remaining').get(function() {
  return {
    ...
    carbohydrates: Math.max(0, this.goals.carbohydrates - this.totalCarbohydrates),
...
dailyNutritionSchema.methods.addMeal = function(mealType: string, recipeId: string, servings: number, nutrition: any) {
...
dailyNutritionSchema.statics.getNutritionStreak = async function(userId: string) {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const dayRecord = records.find(r => 
      r.date.getTime() === checkDate.getTime()
    );
...
dailyNutritionSchema.statics.getWeeklySummary = async function(userId: string, startDate?: Date) {
