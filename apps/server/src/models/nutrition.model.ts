	...
	micronutrients.forEach((nutrient) => {
		if (this[nutrient]) {
			scaledNutrition[nutrient] =
				Math.round(this[nutrient] * scalingFactor * 10) / 10;
		}
	});
...
nutritionSchema.statics.getDailyValuePercentages = (nutritionData: any) => {
	...
	Object.keys(dailyValues).forEach((nutrient) => {
		if (nutritionData[nutrient] && dailyValues[nutrient]) {
			percentages[nutrient] = Math.round(
				(nutritionData[nutrient] / dailyValues[nutrient]) * 100,
			);
		}
	});
...
nutritionSchema.statics.compareNutrition = (
	nutrition1: any,
	nutrition2: any,
) => {
	const comparison = {
		calories: {
			...
			percentDifference: Math.round(
				((nutrition2.calories - nutrition1.calories) / nutrition1.calories) *
					100,
			),
		},
		protein: {
			...
			difference:
				Math.round((nutrition2.protein - nutrition1.protein) * 10) / 10,
			percentDifference: Math.round(
				((nutrition2.protein - nutrition1.protein) / nutrition1.protein) * 100,
			),
		},
		carbohydrates: {
			...
			difference:
				Math.round((nutrition2.carbohydrates - nutrition1.carbohydrates) * 10) /
				10,
			percentDifference: Math.round(
				((nutrition2.carbohydrates - nutrition1.carbohydrates) /
					nutrition1.carbohydrates) *
					100,
			),
		},
		fat: {
			...
			percentDifference: Math.round(
				((nutrition2.fat - nutrition1.fat) / nutrition1.fat) * 100,
			),
		},
	};
