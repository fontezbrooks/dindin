...
const shoppingListSchema = new Schema({
  ...
  templateCategory: {
    type: String,
    enum: ['weekly-essentials', 'monthly-bulk', 'special-occasion', 'dietary-specific', 'budget-friendly'],
  },
  tags: [{
    type: String,
    lowercase: true,
...
shoppingListSchema.virtual('purchasedItems').get(function() {
  return this.items ? this.items.filter((item: any) => item.isPurchased).length : 0;
});
...
shoppingListSchema.methods.purchaseItem = function(itemIndex: number, actualPrice?: number, store?: string) {
...
shoppingListSchema.methods.optimizeShoppingRoute = function() {
  ...
  this.preferredStores
    ...
    .forEach((store: any) => {
      const storeCategories = store.categories.filter((cat: string) => itemsByCategory[cat]);
  ...
  this.shoppingRoute = {
    ...
    totalEstimatedTime: storeRoute.reduce((total: number, store: any) => total + store.estimatedTime, 0),
  };
...
shoppingListSchema.statics.generateFromMealPlan = async function(mealPlanId: string, userId: string, options: any = {}) {
  const MealPlan = model('MealPlan');
  const Recipe = model('Recipe');
  
  const mealPlan = await MealPlan.findById(mealPlanId).populate('dailyPlans.meals.breakfast.recipeId dailyPlans.meals.lunch.recipeId dailyPlans.meals.dinner.recipeId dailyPlans.meals.snacks.recipeId');
  ...
  for (const dayPlan of mealPlan.dailyPlans) {
    ...
    const mealsToProcess = [
      ...
      ...dayPlan.meals.snacks.map((snack: any, index: number) => ({ meal: snack, type: `snack-${index}` })),
    ];
      ...
      if (meal && meal.recipeId) {
        ...
        if (recipe) {
          ...
          recipe.ingredients.forEach((ingredient: any) => {
            ...
            const servingMultiplier = (meal.plannedServings || 1) / (recipe.servings || 1);
...
shoppingListSchema.statics.categorizeIngredient = function(ingredientName: string): string {
  const name = ingredientName.toLowerCase();
  
  // Produce
  if (/\b(apple|banana|orange|lettuce|tomato|onion|garlic|carrot|potato|spinach|broccoli|bell pepper|cucumber|mushroom|avocado)\b/.test(name)) {
    return 'produce';
  }
  
  // Meat & Seafood
  if (/\b(chicken|beef|pork|fish|salmon|shrimp|turkey|ham|bacon|sausage)\b/.test(name)) {
    return 'meat-seafood';
  }
  
  // Dairy & Eggs
  if (/\b(milk|cheese|yogurt|butter|cream|egg|sour cream|cottage cheese)\b/.test(name)) {
    return 'dairy-eggs';
  }
  
  // Pantry
  if (/\b(rice|pasta|bread|flour|sugar|oil|vinegar|pasta|beans|lentils|quinoa|oats)\b/.test(name)) {
    return 'pantry';
  }
  
  // Spices & Seasonings
  if (/\b(salt|pepper|paprika|cumin|oregano|basil|thyme|rosemary|cinnamon|vanilla)\b/.test(name)) {
