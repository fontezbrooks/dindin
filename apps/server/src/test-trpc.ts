...
async function testTRPCImplementation() {
  console.log("🧪 Testing tRPC Implementation...\n");
  ...
  try {
    ...
    await DindinUser.deleteMany({ email: { $in: ["test@example.com", "partner@example.com"] } });
    ...
    console.log("✅ Created 2 test recipes\n");

    // Test 1: User Router - Get/Create Profile
    console.log("🧪 Test 1: User Router - Get/Create Profile");
    ...
    console.log("✅ Partner code generated:", partnerCodeResult.code);

    // Test 3: Create Partner Profile
    console.log("\n🧪 Test 3: Create Partner Profile");
    const partnerProfile = await caller2.user.getProfile();
    console.log("✅ Partner profile created:", partnerProfile.name);

    // Test 4: Connect Partners
    console.log("\n🧪 Test 4: Connect Partners");
    console.log("   Using partner code:", partnerCodeResult.code);
    
    // Let's check if user 1 actually has the partner code
    const user1Check = await DindinUser.findOne({ authUserId: mockSession.user.id });
    console.log("   User 1 partner code in DB:", user1Check?.partnerCode);
    ...
    console.log(`✅ Retrieved ${recipeStack.length} recipes in stack`);
    ...
    console.log("✅ User 1 liked recipe, matched:", likeResult1.matched);
    ...
    console.log("✅ User 2 liked recipe, matched:", likeResult2.matched);
    if (likeResult2.matched) {
      console.log("🎉 Match created! Match ID:", likeResult2.matchId);
    }
    ...
    console.log(`✅ Retrieved ${matches.matches.length} matches`);
    if (matches.matches.length > 0) {
      console.log("   First match recipe:", matches.matches[0].recipeId.title);
    }
    ...
    if (matches.matches.length > 0) {
      ...
      console.log("✅ Match status updated to:", updateResult.status);
    }
    ...
    console.log(`✅ Found ${searchResults.recipes.length} Italian recipes under 45 minutes`);
    ...
    console.log("✅ Recipe disliked, matched:", dislikeResult.matched);
